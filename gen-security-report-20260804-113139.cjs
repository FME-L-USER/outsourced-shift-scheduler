...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-113139.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表管理 — 新增「修正一週一例」批次修正功能）：'),
  body('  新增 handleFixWeeklyEx 函式，掃描現有班表資料：'),
  body('  1. 對每位員工，收集所有「例」代號日期並依日期排序'),
  body('  2. 以 ISO 週（週一為第一天）為單位，保留每週第一個「例」'),
  body('  3. 同週第 2 個以上的「例」自動降格為「休」'),
  body('  4. 透過 setSchedule 批次更新 React state，顯示修正格位數'),
  body('  新增「🔧 修正一週一例」按鈕於班表管理工具列（匯入班表旁）'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 state 批次運算，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  handleFixWeeklyEx 僅讀取既有 schedule state 與 visibleEmployees，'),
  body('  不接受任何使用者輸入字串，無 XSS 或注入風險。'),
  body('  以 ts（毫秒）排序後做 weekExSeen Set 查找，演算法正確且複雜度 O(n log n)。'),
  body('  setSchedule 為既有 React setState，不引入新的儲存路徑或認證繞過風險。'),
  body('  不引入新的 API 路徑、認證變更或資料暴露風險。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  批次修正現有班表資料，每週最多一個「例」'),
