...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-155718.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（回報作業組別下拉選單修正）：'),
  body('  1. groupOptions 來源新增 employees.group 欄位'),
  body('     原：只收集 e.shiftType、extras.group、attendSettings.groups'),
  body('     改：額外加入 employees.map(e => e.group).filter(Boolean) 去重合併'),
  body('     原因：員工有獨立的 group 欄位（作業組別），與 shiftType（班別）不同'),
  body('     修正：部分員工的 group 值未出現在回報作業組別下拉選單中'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 useMemo 資料來源擴充，無安全影響，可直接部署。'),
  ...
  body('  \u2705  回報作業組別下拉選單現可顯示所有員工的 group 欄位值'),
