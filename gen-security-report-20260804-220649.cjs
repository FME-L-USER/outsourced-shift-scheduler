...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-220649.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：各廠商到班率圖表由橫條圖改為直條圖'),
  body('  - 每廠商顯示兩組直條：青色（長期到班率）、橙色（臨時到班率）'),
  body('  - 使用 Canvas 2D API 繪製，無外部資料請求，純前端計算'),
  body('  - roundRectV 繪製圓角頂部直條，底部切齊基線'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 純視覺改版，無新增 API 呼叫或資料存取'),
  body('  - Canvas ctx.fillText 無 XSS 風險'),
  body('  - 未修改任何後端邏輯'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端圖表樣式調整，無安全疑慮，可直接部署。'),
  ...
  body('  \u2705  Canvas 直條圖繪製，無 XSS 風險'),
  body('  \u2705  無新增 API 端點或後端異動'),
]}]});
