...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-165900.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（切回前景自動同步最新資料）：'),
  body('  問題：同事 A 儲存資料後，同事 B 的瀏覽器不會自動更新'),
  body('        React state 是頁面載入時的快照，無即時同步機制'),
  body('  解法：監聽 document.visibilitychange 事件'),
  body('     - 切回前景（document.visibilityState === "visible"）時'),
  body('       admin/area：呼叫 GET /api/state 並合併最新資料至 React state'),
  body('       vendor：呼叫 GET /api/attendance 並合併 attendData/extras'),
  body('     - 僅在 serverSyncedRef 已完成初始同步後才觸發'),
  body('     - 使用 catch(() => {}) 靜默處理錯誤，不中斷使用者操作'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - visibilitychange 為純瀏覽器事件，不引入新攻擊面'),
  body('  - fetch 請求仍攜帶 JWT Bearer token，後端驗證不變'),
  body('  - 只做「讀取合併」，不觸發寫入，不影響現有 debounce 存檔邏輯'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端事件監聽，無安全影響，可直接部署。'),
  ...
  body('  \u2705  切回分頁時自動從後端拉取最新資料，同事間資料即時同步'),
