...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-154423.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（點名表回報作業組別選擇修正）：'),
  body('  1. groupOptions 納入臨時人員 extras 的 group 欄位'),
  body('     原：只從 employees.shiftType 及 attendSettings.groups 取得'),
  body('     改：額外加入 Object.values(extras).flat().map(e => e.group) 去重合併'),
  body('     修正：派工匯入的臨時人員有 group 值，但不出現在回報作業組別下拉選單中'),
  body('  2. ReportPane 元件從 Attendance 內部定義移至模組層級'),
  body('     同 MaintPane 修正原因：避免父元件 re-render 時 unmount/remount 造成 state 重設'),
  body('     generateReport、groupOptions、attendDate、groupFilter 改為 props 傳入'),
  ...
  body('補充說明：groupOptions 擴充來源仍為前端本地資料，不引入新 API 路徑或資料暴露。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  回報作業組別下拉選單現可顯示臨時人員（派工匯入）的組別'),
  body('  \u2705  ReportPane 移至模組層級，修正 state 重設問題'),
