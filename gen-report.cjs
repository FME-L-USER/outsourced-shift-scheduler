...
const findings = [
  ...
  ['Medium','課別鎖定與開放排班區間只在前端把關','OWASP A01','已修復',
   '管理員將課別改為鎖定後，已登入且未重新整理的委外人員／幹部仍可繼續排班，且送出的內容會被伺服器接受。PUT /api/state 的 vendor/worker 分支僅驗證廠商歸屬，未檢查 deptLocks 與 deptRanges，等同於鎖定可被繞過（無需刻意攻擊，正常操作即會發生）。',
   'server.cjs / PUT /api/state（vendor/worker 分支）',
   '改由伺服器最終認定：逐筆檢查課別鎖定模式（部分鎖定、全部鎖定一律拒收）與每一格日期是否落在該課開放排班區間內，被拒格數寫入日誌並回傳前端；前端收到後提示使用者並立即重新同步設定。'],
  ['Low','國定假日排班的數量上限從未生效','—','已修復',
   '上限檢查在比對班表時將日期補零（2026-10-05），但班表的鍵不補零（2026-10-5），因此永遠查不到已排定的「國」，數量上限形同虛設。',
   'ShiftSystem.jsx / handleCellClick',
   '改以本期實際的日期鍵比對，並將額度定義為「本期區間內已勾選的國定假日天數」，與「一鍵轉換國」使用同一套規則。'],
