...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260806-152733.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：修正倉別篩選無課別設定時錯誤過濾員工導致綠色到班率顯示 0%'),
  spacer(),
  h2('問題描述'),spacer(),
  body('問題現象：儀表板選擇倉別後，長期到班率（綠色橫條）顯示 0%，即使點名表已打勾。'),
  spacer(),
  body('根本原因：'),
  body('  filterByScope 在倉庫有找到（wh != null）但 wh.departments 為空陣列時，'),
  body('  建立空的 deptNames / whVendors，導致所有員工被過濾掉（dashEmployees = []），'),
  body('  vendorStats 的 working = 0，longRate = longPresent / 0 = 0，綠色橫條不顯示。'),
  spacer(),
  h2('修正內容'),spacer(),
  body('修改位置：ShiftSystem.jsx filterByScope 函式（第 508～518 行）'),
  body('修前：if (wh) { ... filter ... }'),
  body('修後：if (wh && wh.departments.length > 0) { ... filter ... }'),
  body('  倉庫未設定課別時直接跳過過濾，顯示全部員工，符合業務需求。'),
  ...
  body('  ✅  filterByScope 加入 departments.length > 0 判斷'),
  body('  ✅  倉庫無課別設定時不過濾，綠色長期到班率可正確計算'),
]}]});
