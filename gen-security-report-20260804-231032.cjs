...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-231032.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：修復點名表維護功能無法輸入的問題'),
  body('  根本原因：TagList / AddRow 元件定義在 MaintPane 函式內部'),
  body('  每次 state 更新觸發 re-render 時，React 視為新元件型別'),
  body('  導致 input 被 unmount/remount，游標每打一字就跳出'),
  body('  修復方式：將 MaintTagList / MaintAddRow 移至 MaintPane 函式外部'),
  body('  React 元件參考穩定後，input 可正常連續輸入'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 純重構，不影響任何資料存取或權限控制'),
  body('  - localStorage 存取邏輯不變'),
  ...
  body('  \u2705  MaintTagList / MaintAddRow 移至模組層級，元件參考穩定'),
  body('  \u2705  維護頁面輸入框可正常連續打字新增項目'),
]}]});
