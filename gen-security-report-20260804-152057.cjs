...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-152057.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（側邊欄配色由綠色改為深海軍藍）：'),
  body('  1. --sms-sidebar:       #14302D → #1a2f5e（深海軍藍）'),
  body('  2. --sms-sidebar-hover: #1C4A46 → #1e3870（hover 狀態藍）'),
  body('  3. --sms-sidebar-active:#1E7A4A → #2563eb（選中項目亮藍）'),
  body('     純 CSS 色彩調整，不影響任何功能邏輯'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純外觀色彩調整，無安全影響，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：本次修改為純 CSS 自訂屬性（--sms-sidebar 系列）色彩值調整，'),
  body('不引入新邏輯、不變更任何 API 路徑或驗證機制。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  側邊欄改為深海軍藍配色（#1a2f5e）'),
