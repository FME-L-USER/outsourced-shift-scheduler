...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-152744.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（全系統按鈕配色由綠色改為深海軍藍）：'),
  body('  1. CSS 變數更新：'),
  body('     --sms-teal:       #0F766E → #1a2f5e'),
  body('     --sms-teal-dark:  #0D6660 → #1e3870'),
  body('     --sms-teal-light: #CCFBF1 → #dbeafe'),
  body('     --sms-teal-50:    #F0FDFA → #eff6ff'),
  body('  2. Tailwind 按鈕類別全域替換（共 9 處）：'),
  body('     bg-teal-700 → bg-[#1a2f5e]'),
  body('     bg-teal-600 → bg-[#1e3870]'),
  body('     hover:bg-teal-800 → hover:bg-[#1e3870]'),
  body('     hover:bg-teal-700 → hover:bg-[#1a2f5e]'),
  body('  影響範圍：登入頁「登入系統」按鈕、點名表、班表存檔等各主要操作按鈕'),
  ...
  body('補充說明：本次修改為純 CSS 色彩值調整，不引入新邏輯或 API 路徑。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  全系統按鈕配色統一改為深海軍藍，與側邊欄一致'),
