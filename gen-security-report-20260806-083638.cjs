...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260806-083638.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：修復同步後資料仍顯示舊值（改為 server 優先 merge）'),
  body('  功能說明：'),
  body('    - syncFromServer 的物件展開順序從 { ...newAttend, ...prev } 改為 { ...prev, ...newAttend }'),
  body('    - 原本 local（prev）優先蓋掉 server 資料，導致同步後仍顯示本機舊值'),
  body('    - 修正後 server 資料優先，同步即可看到其他裝置最新勾選'),
  body('    - extras 同步邏輯亦同步修正'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 僅調換物件展開順序，不涉及任何 API 端點或權限變更'),
  body('  - 不新增任何輸入來源或資料流路徑'),
  body('  - server 優先 merge 符合最小特權原則：本機資料不應蓋過已驗證的 server 資料'),
  ...
  body('  ✅  syncFromServer 改為 server 優先 merge，同步後桌機與手機顯示一致'),
  body('  ✅  不影響任何既有安全機制（JWT 驗證、scope 過濾、employee-level merge）'),
]}]});
