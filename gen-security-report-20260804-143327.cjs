...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-143327.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（修正委外人員登入 — 員編比對加入 trim）：'),
  body('  1. LoginScreen handleSubmit worker 登入邏輯（line 733）'),
  body('     原：employees.find(em => em.empId === username)'),
  body('     改：employees.find(em => em.empId?.trim() === username.trim())'),
  body('     原因：username 輸入框未 trim，若有前後空白則比對失敗回傳「員工編號不存在」'),
  body('     em.empId 加 ?. 防範員工資料中 empId 為 undefined 的邊界情況'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純字串比較微調，無任何安全影響，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  trim() 不影響驗證強度：員編為識別碼而非密碼，去除空白為合理的 UX 容錯。'),
  body('  密碼驗證（verifyPwd / password === emp.empId）未變更，安全性不受影響。'),
  body('  不引入新的 API 路徑、認證邏輯或資料暴露路徑。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  委外人員員編比對兩端加入 trim()，修正輸入空白導致無法登入的問題'),
  body('  \u2705  em.empId?. 防範 empId 為 undefined 的邊界情況'),
  body('  \u2705  密碼驗證邏輯未變更，安全性不受影響'),
