...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260806-084255.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：admin/area 出勤資料改走 PUT /api/attendance 存檔'),
  body('  問題描述：'),
  body('    - PUT /api/state（server 端）明確 strip attendData/extras，不存入 DB'),
  body('    - 出勤自動存檔 effect 原本只允許 VENDOR 角色執行'),
  body('    - 導致 admin/area 在手機勾選的出勤資料永遠只停在 localStorage，從未存進 DB'),
  body('    - 結果：不同裝置登入同一 admin/area 帳號，出勤資料無法同步'),
  body('  修正方式：'),
  body('    - role guard 從 role !== ROLES.VENDOR 改為白名單：VENDOR/ADMIN/AREA 皆允許'),
  body('    - admin/area 出勤變更後亦會走 PUT /api/attendance（2s debounce）存入 DB'),
  body('    - PUT /api/attendance server 端對 admin/area 無 scope 限制，行為正確'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 明確白名單（VENDOR/ADMIN/AREA），WORKER 及未登入者仍不觸發存檔'),
  body('  - PUT /api/attendance 已有 requireAuth 保護，server 端亦驗證 role'),
  body('  - 不新增任何端點或權限，僅修正前端呼叫時機'),
  ...
  body('  ✅  admin/area 出勤資料現可正確存入 DB（透過 PUT /api/attendance）'),
  body('  ✅  跨裝置同步問題根因修復：手機勾選 → DB → 桌機同步'),
  body('  ✅  role 白名單收緊，非授權角色不觸發存檔'),
]}]});
