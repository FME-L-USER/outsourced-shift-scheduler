...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-095443.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（帳號與權限管理 — 依上方條件篩選）：'),
  body('  AccountManagement 元件新增支援上方篩選列（倉別/課別/組別）：'),
  body('  1. useApp() 解構加入 selectedWarehouse、selectedDept、selectedGroup、employees'),
  body('  2. 廠商帳號 tab 新增 scopeVendorNames，依選取倉別/課別限縮可見廠商帳號'),
  body('  3. 委外人員 tab 的 workerEmpListAll 改用 filterByScope() 過濾員工清冊'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 state 篩選，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  selectedWarehouse/Dept/Group 來自 AppContext（useApp()），由 React state 提供。'),
  body('  filterByScope() 為既有工具函式，已在 ScheduleTable 等元件中使用。'),
  body('  scopeVendorNames 僅讀取 warehouses 靜態結構，不涉及使用者輸入。'),
  body('  不引入新的 API 路徑、認證變更或資料暴露風險。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  純前端 state 篩選，無 XSS 或注入風險'),
  body('  ✅  filterByScope() 為既有函式，不引入新邏輯'),
