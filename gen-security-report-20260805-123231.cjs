...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260805-123231.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：修復維護設定（attendSettings）在切換分頁後未同步到其他電腦的問題'),
  body('  根本原因：applySchedule 函式（切回前景自動同步）未套用 attendSettings'),
  body('  loadServerState（初次登入）已正確讀取 attendSettings，'),
  body('  但 visibilitychange 觸發的重新整理只更新 employees/schedule 等欄位'),
  body('  attendSettings 被排除在外，導致不同電腦切換分頁時看不到最新維護設定'),
  body('  修復方式：在 applySchedule 中補上 if (s.attendSettings) setAttendSettings(s.attendSettings)'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 純前端狀態讀取，從已通過驗證的 GET /api/state 回應中取值'),
  body('  - 不涉及任何新的 API 呼叫或資料寫入'),
  body('  - 不擴大任何角色的資料存取權限'),
  ...
  body('  \u2705  applySchedule 補上 attendSettings，切回前景時維護設定可即時同步'),
  body('  \u2705  與 loadServerState 初次載入邏輯統一，消除兩處不一致'),
  body('  \u2705  不同電腦或不同使用者切換分頁後可看到最新維護資料'),
]}]});
