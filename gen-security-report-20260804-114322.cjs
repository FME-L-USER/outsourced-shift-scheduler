...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-114322.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表管理 — 新增「休假天數」欄位）：'),
  body('  在「出勤天」欄位右方新增「休假天」欄位：'),
  body('  1. 表頭新增 <th>休假天</th>，緊接在「出勤天」之後'),
  body('  2. 每位員工的 leaveDays 計數：code === 休 / 例 / 國 時累加'),
  body('  3. 資料列新增 <td>，橘色字體顯示休假天數'),
  body('  4. 底部統計列補上對應空白 <td>，確保欄位對齊'),
  ...
  body('補充說明：'),
  body('  leaveDays 僅累計已存在 schedule state 中的代號（休/例/國），不接受使用者輸入。'),
  body('  純前端 state 衍生計算，無 XSS 或注入風險。'),
  body('  不引入新的 API 路徑、認證變更或資料暴露風險。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  班表管理新增休假天數欄位，橘色字體顯示'),
