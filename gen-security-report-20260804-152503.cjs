...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-152503.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（點名表臨時人員卡新增全選功能）：'),
  body('  1. 臨時人員橘色 header 右側新增「全選」checkbox'),
  body('     - 全部勾選時：checkbox 呈勾選狀態'),
  body('     - 部分勾選時：checkbox 呈 indeterminate 狀態'),
  body('     - 全部未勾選時：checkbox 空白'),
  body('     - 點擊全選：對該廠商所有臨時人員呼叫 setExtraRecord({ present: true/false })'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純 UI 功能新增，無安全影響，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：全選操作僅修改前端 extras state（本地點名記錄），'),
  body('不引入新 API 路徑、不繞過任何驗證邏輯。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  臨時人員卡新增全選 checkbox，支援 indeterminate 部分選取狀態'),
