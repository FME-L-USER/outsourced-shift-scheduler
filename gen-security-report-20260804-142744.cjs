...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-142744.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（修正班表管理白屏 bug — handlePrintReport TDZ 問題）：'),
  body('  1. 將 handlePrintReport useCallback 從 dayHeaders 定義之前（line 2403）'),
  body('     移至 dayHeaders useMemo 定義之後（line 2477）'),
  body('     原因：handlePrintReport deps array 引用了 dayHeaders，'),
  body('     但 dayHeaders 以 const 宣告在後方，造成 Temporal Dead Zone ReferenceError，'),
  body('     React render 時直接 crash 導致班表管理頁白屏'),
  body('  2. 純程式碼順序調整，handlePrintReport 函式本體內容完全未變更'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純結構性調整，無任何安全異動，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  本次修改為純程式碼順序調整（移動 useCallback 定義位置），'),
  body('  不引入任何新邏輯、新輸入點或新輸出路徑。'),
  body('  handlePrintReport 函式本體（XSS 防護、window.open、HTML 產生邏輯）完全未變更。'),
  body('  不引入新的 API 路徑、認證邏輯或資料暴露路徑。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  修正 handlePrintReport 定義順序（移至 dayHeaders 之後）'),
  body('  \u2705  解決 const TDZ ReferenceError 導致的班表管理白屏問題'),
  body('  \u2705  純結構性調整，無 XSS 或注入風險變化'),
