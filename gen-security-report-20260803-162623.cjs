...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260803-162623.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表管理表格容器高度與底部留白調整）：'),
  body('  1. maxHeight 由 calc(100vh - 260px) 改為 calc(100vh - 220px)，表格可視區域增加 40px'),
  body('  2. 新增 pb-4（16px）底部內距，使統計列底部留有呼吸空間'),
  ...
  body('補充說明：'),
  body('  maxHeight 為純 CSS 計算值，僅影響版面呈現，不引入任何攻擊面。'),
  body('  pb-4 為 Tailwind CSS utility class，不引入任何 JavaScript 邏輯或資料處理。'),
