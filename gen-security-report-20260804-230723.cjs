...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-230723.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：各廠商人力明細表格移除「在職」欄位'),
  body('  - 移除 thead 在職 th 及對應 colSpan 調整'),
  body('  - 移除 vendor row 及合計 row 的 roster td'),
  body('  - 純前端 UI 調整，無資料存取異動'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 純 UI 欄位隱藏，無新增 API 呼叫'),
  body('  - 不影響後端資料或權限控制'),
  ...
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),
]}]});
