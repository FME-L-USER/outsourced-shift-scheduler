...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260803-153734.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表匯入未匹配員編顯示改善）：'),
  body('  原本 toast 只顯示「N 筆員編未匹配」，使用者無法得知是哪幾筆。'),
  body('  1. 新增 unmatchedIds 陣列，收集每筆未匹配的員編或姓名（含廠商）'),
  body('  2. Toast 訊息改為顯示具體未匹配識別碼，超過 10 筆時截斷並標示總數'),
  body('     格式：（未匹配：A001、王小明（日翊）…等N筆）'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 UI 訊息改善，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  identifier 來自 Excel 匯入檔（使用者本機檔案），為純字串。'),
  body('  Toast 在 React 中渲染，React 自動 escape 字串，不存在 XSS 風險。'),
  body('  unmatchedIds 陣列僅用於顯示，不寫入 DB 或任何持久化儲存。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  identifier 字串經 React 自動 escape，無 XSS 風險'),
  body('  ✅  不引入新的 API 路徑、DB 查詢或認證變更'),
  body('  ✅  不引入任何資料外洩風險'),
]}]});
