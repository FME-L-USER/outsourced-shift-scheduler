...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-115109.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表管理 — 匯出 Excel 新增休假天數欄位）：'),
  body('  exportConverted 函式（代碼轉換版班表匯出）異動：'),
  body('  1. header 陣列末尾加入「休假天」欄位名稱'),
  body('  2. 每列計算 leaveDays（raw === 休 / 例 / 國 時累加）'),
  body('  3. 每列 row 陣列末尾附加 leaveDays 數值'),
  body('  4. ws["!cols"] 對應補上 { wch: 6 } 欄寬'),
  body('  exportScheduleRaw（純備份格式，供重新匯入）不異動。'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 state 計算後寫入 XLSX，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  leaveDays 僅讀取 schedule state 代號，不接受使用者輸入，無 XSS 或注入風險。'),
  body('  XLSX.writeFile 為既有呼叫，不引入新的資料暴露或認證風險。'),
  body('  不引入新的 API 路徑、認證變更或資料暴露風險。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  匯出 Excel 新增休假天數欄位（休＋例＋國）'),
