...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-150509.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（修正 ScheduleTable 白屏 — isWorker 變數未定義）：'),
  body('  1. ScheduleTable 函式（line 1898）中補上 isWorker 定義'),
  body('     原：isWorker 未在 ScheduleTable scope 內定義'),
  body('     改：在 shiftTypes 定義前加入 const isWorker = currentUser.role === ROLES.WORKER'),
  body('     原因：上一版新增的 !isWorker 工具列隱藏條件引用了未定義變數，'),
  body('     導致委外人員登入後班表管理頁面白屏（ReferenceError）'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：修正未定義變數，無安全影響，可直接部署。'),
  ...
  body('  \u2705  ScheduleTable 補上 isWorker 定義，修正白屏問題'),
