...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-155145.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（修正一週一例功能 handleFixWeeklyEx 雙向修正）：'),
  body('  1. 原邏輯：只處理週內 2+ 個「例」→ 多餘的例改為休'),
  body('     新邏輯（雙向）：'),
  body('     ① 週內 >1 個「例」→ 保留第 1 個，其餘改「休」（原有）'),
  body('     ② 週內 0 個「例」但有「休」→ 最後一個「休」改為「例」（新增）'),
  body('  2. 實作方式：'),
  body('     - 收集每位員工所有例/休日，依週（週一為起點）分組'),
  body('     - 對每個週分組分別處理兩種情況，empFixed 旗標追蹤是否有異動'),
  body('     - 0 例 + 有休 → 取 restDays 最後一個（週日位置）改為例'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純排班邏輯調整，無安全影響，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：handleFixWeeklyEx 為純前端資料轉換函式，'),
  body('操作對象為本地 schedule state，不引入新 API 路徑或資料暴露。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  修正一週一例功能現支援雙向修正（補例 + 去多餘例）'),
