...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-103643.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（Dashboard 各廠商人力組別欄位動態化）：'),
  body('  問題：GROUP_COLS 硬寫死關鍵字比對（如必須同時含「日班」+「理貨」），'),
  body('        導致清冊中非此命名格式的組別（如「理貨組」「大溪理貨一課」）'),
  body('        無法命中對應欄位，只有條件最寬鬆的「日班出貨組」有資料。'),
  body('  修正：GROUP_COLS 改由 visibleEmployees 的 group 欄位動態產生，'),
  body('        以員工清冊實際組別名稱為欄位標題，精確比對（empGroup === g），'),
  body('        日班優先、中班次之，其餘依字典序排列。'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 useMemo 運算，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  GROUP_COLS 由 visibleEmployees（React state）衍生，不涉及使用者輸入。'),
  body('  組別名稱以 Set 去重，再排序後 map 為精確比對函式，無 XSS 或注入風險。'),
  body('  不引入新的 API 路徑、認證變更或資料暴露風險。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  動態 GROUP_COLS，不依賴硬寫死關鍵字，正確反映清冊實際組別'),
  body('  ✅  純 React state 衍生，無 XSS 或注入風險'),
