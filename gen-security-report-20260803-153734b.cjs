...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260803-154200.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（班表管理底部新增統計列）：'),
  body('  在班表管理 <tbody> 底部新增三列統計行：'),
  body('  1. 總人數 — visibleEmployees.length（每格顯示相同數字）'),
  body('  2. 出勤人數 — 計算每日班表值為「V」的員工數'),
  body('  3. 休假人數 — 計算每日班表值為「休/例/國」的員工數'),
  body('  三列依序以灰/綠/橙底色區分，字體加粗以利閱讀。'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：純前端 UI 改善，無任何漏洞，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  統計列僅讀取已在 React state 的 schedule / visibleEmployees，無任何 API 呼叫。'),
  body('  fn(dk) 回傳純數字，直接渲染為文字，無 XSS 風險。'),
  body('  不引入任何新的資料存取路徑或認證變更。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  ✅  純前端統計計算，讀取已有 React state，無新攻擊面'),
  body('  ✅  回傳值為數字，無 XSS 風險'),
