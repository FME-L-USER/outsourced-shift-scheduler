...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-233203.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  h1('1. 執行摘要'),spacer(),
  body('掃描範圍：server.cjs（後端 API）'),
  body('本次異動：修復手機登入只有臨時到班率、無長期到班率的問題'),
  body('  根本原因：PUT /api/state 使用 SET data=$1 全量覆寫'),
  body('  vendor 透過 PUT /api/attendance 存入的 attendData/extras 會被覆蓋'),
  body('  area 角色存檔時，若本地 attendData 是舊的（vendor 已更新 DB），'),
  body('  area 一存就覆蓋 DB，手機重新登入後讀到空的 attendData，長期圖消失'),
  body('  修復方式：PUT /api/state 改為 JSONB merge，先移除 attendData/extras，'),
  body('  再用 app_state.data || $1::jsonb 合併，永遠保留 vendor 存入的出勤資料'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - PUT /api/state 仍受 requireAuth + requireManagerOrAdmin 保護'),
  body('  - 只有 admin/area 角色可呼叫，vendor/worker 無法存取此 endpoint'),
  body('  - 移除 attendData/extras 防止 area 意外傳入惡意出勤資料覆蓋 vendor 記錄'),
  body('  - JSONB merge（||）為 PostgreSQL 原生操作，無 SQL injection 風險'),
  ...
  body('  \u2705  PUT /api/state 改用 JSONB merge，保留 attendData/extras'),
  body('  \u2705  移除 attendData/extras 欄位防止 area 角色意外覆蓋出勤資料'),
  body('  \u2705  長期到班率（長）在 area/手機 登入後可正確顯示'),
]}]});
