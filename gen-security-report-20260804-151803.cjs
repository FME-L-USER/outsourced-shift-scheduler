...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260804-151803.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動（修正點名表維護頁輸入欄位無法打字）：'),
  body('  1. MaintPane 元件從 Attendance 內部定義移至模組層級'),
  body('     原因：MaintPane 定義在父元件 Attendance 函式內部，'),
  body('     每次父元件 re-render（如 state 變更）就會重建 MaintPane 的 function 參考，'),
  body('     React 將其視為全新元件並 unmount/remount，'),
  body('     導致輸入框每打一個字就失去焦點（無法連續輸入）'),
  body('  2. 修復方式：'),
  body('     - MaintPane 移至 function Attendance() 定義前的模組層級'),
  body('     - attendSettings、setAttendSettings、groupOptions 改為 props 傳入'),
  body('     - attendData、setAttendData、employees 改從 useApp() 取得'),
  body('     - toast 改從 useToast() 取得'),
  spacer(),body('漏洞統計：',{bold:true}),spacer(),statsTable,spacer(),
  body('部署建議：React 元件結構修正，無安全影響，可直接部署。'),
  spacer(),new Paragraph({children:[new PageBreak()]}),
  h1('2. 漏洞詳細清單'),spacer(),body('本次掃描未發現任何漏洞。'),spacer(),
  body('補充說明：'),
  body('  本次為純元件結構調整，不引入新的 API 路徑、認證邏輯或資料暴露路徑。'),
  body('  MaintPane 的資安屬性不變：attendSettings 僅存於 localStorage，'),
  body('  attendData 清除操作需要使用者手動觸發並二次確認。'),
  spacer(),
  new Paragraph({children:[new PageBreak()]}),
  h1('3. 結論'),spacer(),
  body('本次掃描未發現任何漏洞，程式碼通過資安檢測，允許部署。'),spacer(),
  body('  \u2705  MaintPane 移至模組層級，修正維護頁輸入欄位無法打字問題'),
