...
const fs = require('fs');
const OUTPUT = 'C:\\Users\\Grace\\Desktop\\AI測試好朋友\\輪休表\\排班作業\\security-report-20260805-134854.docx';
...
const doc = new Document({ sections:[{ properties:{ page:{ size:{width:11906,height:16838}, margin:{top:1440,right:1440,bottom:1440,left:1440} } }, children:[
  ...
  body('本次異動：修復廠商幹部（VENDOR）無法看到倉別/課別/組別篩選選單的問題'),
  body('  根本原因：WarehouseDeptBar 的 visibleWarehouses 邏輯'),
  body('  僅對 ADMIN 顯示全部倉別，VENDOR 因無 allowedWarehouses 而得到空陣列'),
  body('  導致倉別下拉空白，組別選單永遠無法展開，看不到中班等組別'),
  body('  修復方式：在條件中加入 isVendor，讓 VENDOR 與 ADMIN 同樣看到全部倉別'),
  body('  資料邊界不變：各頁面仍由 filterByScope + currentUser.vendors 過濾，'),
  body('  VENDOR 只能看到自己廠商的員工資料，不擴大任何存取權限'),
  spacer(),body('安全性分析：',{bold:true}),
  body('  - 純前端 UI 顯示邏輯，不影響任何 API 呼叫或資料存取'),
  body('  - filterByScope 與各頁面的 vendor 過濾邏輯完整保留'),
  body('  - VENDOR 看到組別選單後，篩選結果仍限於自己廠商的員工'),
  body('  - 不涉及跨廠商資料洩漏風險'),
  ...
  body('  \u2705  VENDOR 可正常看到倉別、課別、組別篩選選單（含中班等組別）'),
  body('  \u2705  資料邊界未改變，filterByScope 仍限制 VENDOR 只看自己廠商員工'),
  body('  \u2705  ADMIN/AREA 行為不受影響'),
]}]});
